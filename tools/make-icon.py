...
def render(n):
    ...
    # isometric cube vertices (unit coords)
    top  = P(0.50, 0.09)
    ul   = P(0.085, 0.305)
    ur   = P(0.915, 0.305)
    c    = P(0.50, 0.52)
    ...
    ew = max(1, int(n * 0.012))   # edge width

    # faces
    d.polygon([top, ur, c, ul], fill=TOP)      # top
    d.polygon([ul, c, bot, ll], fill=LEFT)     # left/front
    d.polygon([c, ur, lr, bot], fill=RIGHT)    # right/front

    # packing-tape stripe down the front seam (box cue)
    tw = n * 0.10
    d.polygon([lerp(top, ul, 0.5), lerp(top, ur, 0.5),
               lerp(bot, lr, 0.0), lerp(bot, ll, 0.0)], fill=None)
    d.line([lerp(ul, ur, 0.5), c, bot],
           fill=TAPE, width=max(1, int(n * 0.05)))

    # outline: silhouette + inner edges
    for a, b in [(top, ul), (top, ur), (ul, ll), (ur, lr),
                 (ll, bot), (lr, bot), (ul, c), (ur, c), (c, bot)]:
        d.line([a, b], fill=EDGE, width=ew)
